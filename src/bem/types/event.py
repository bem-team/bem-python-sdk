# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from typing import List, Union, Optional
from datetime import datetime
from typing_extensions import Literal, Annotated, TypeAlias

from pydantic import Field as FieldInfo

from .._utils import PropertyInfo
from .._models import BaseModel
from .any_type import AnyType
from .input_type import InputType
from .error_event import ErrorEvent
from .inbound_email_event import InboundEmailEvent
from .send_destination_type import SendDestinationType

__all__ = [
    "Event",
    "TransformEvent",
    "TransformEventCorrectedContent",
    "TransformEventCorrectedContentOutput",
    "TransformEventInput",
    "TransformEventMetadata",
    "TransformEventMetrics",
    "TransformEventMetricsDifference",
    "TransformEventMetricsMetrics",
    "ExtractEvent",
    "ExtractEventCorrectedContent",
    "ExtractEventCorrectedContentOutput",
    "ExtractEventInput",
    "ExtractEventMetadata",
    "ParseEvent",
    "ParseEventCorrectedContent",
    "ParseEventCorrectedContentOutput",
    "ParseEventInput",
    "ParseEventMetadata",
    "AnalyzeEvent",
    "AnalyzeEventMetadata",
    "RouteEvent",
    "RouteEventMetadata",
    "ClassifyEvent",
    "ClassifyEventMetadata",
    "SplitCollectionEvent",
    "SplitCollectionEventPrintPageOutput",
    "SplitCollectionEventPrintPageOutputItem",
    "SplitCollectionEventSemanticPageOutput",
    "SplitCollectionEventSemanticPageOutputItem",
    "SplitCollectionEventMetadata",
    "SplitItemEvent",
    "SplitItemEventMetadata",
    "SplitItemEventPrintPageOutput",
    "SplitItemEventSemanticPageOutput",
    "JoinEvent",
    "JoinEventItem",
    "JoinEventMetadata",
    "EnrichEvent",
    "EnrichEventMetadata",
    "PayloadShapingEvent",
    "PayloadShapingEventMetadata",
    "EvaluationEvent",
    "EvaluationEventMetadata",
    "CollectionProcessingEvent",
    "CollectionProcessingEventMetadata",
    "SendEvent",
    "SendEventGoogleDriveOutput",
    "SendEventMetadata",
    "SendEventS3Output",
    "SendEventWebhookOutput",
    "RenderEvent",
    "RenderEventMetadata",
]


class TransformEventCorrectedContentOutput(BaseModel):
    output: Optional[List[Optional[AnyType]]] = None


TransformEventCorrectedContent: TypeAlias = Union[
    TransformEventCorrectedContentOutput, List[object], str, float, bool, object, None
]


class TransformEventInput(BaseModel):
    input_content: Optional[str] = FieldInfo(alias="inputContent", default=None)

    input_type: Optional[str] = FieldInfo(alias="inputType", default=None)

    json_input_content: Optional[object] = FieldInfo(alias="jsonInputContent", default=None)

    s3_url: Optional[str] = FieldInfo(alias="s3URL", default=None)


class TransformEventMetadata(BaseModel):
    duration_function_to_event_seconds: Optional[float] = FieldInfo(
        alias="durationFunctionToEventSeconds", default=None
    )


class TransformEventMetricsDifference(BaseModel):
    category: Optional[str] = None

    corrected_val: Optional[object] = FieldInfo(alias="correctedVal", default=None)

    extracted_val: Optional[object] = FieldInfo(alias="extractedVal", default=None)

    json_pointer: Optional[str] = FieldInfo(alias="jsonPointer", default=None)


class TransformEventMetricsMetrics(BaseModel):
    accuracy: Optional[float] = None

    f1_score: Optional[float] = FieldInfo(alias="f1Score", default=None)

    precision: Optional[float] = None

    recall: Optional[float] = None


class TransformEventMetrics(BaseModel):
    """Accuracy, precision, recall, and F1 score when corrected JSON is provided."""

    differences: Optional[List[TransformEventMetricsDifference]] = None

    metrics: Optional[TransformEventMetricsMetrics] = None


class TransformEvent(BaseModel):
    event_id: str = FieldInfo(alias="eventID")
    """Unique ID generated by bem to identify the event."""

    function_id: str = FieldInfo(alias="functionID")
    """Unique identifier of function that this event is associated with."""

    function_name: str = FieldInfo(alias="functionName")
    """Unique name of function that this event is associated with."""

    item_count: int = FieldInfo(alias="itemCount")
    """The number of items that were transformed.

    Used for batch transformations to indicate how many items were transformed.
    """

    item_offset: int = FieldInfo(alias="itemOffset")
    """The offset of the first item that was transformed.

    Used for batch transformations to indicate which item in the batch this event
    corresponds to.
    """

    reference_id: str = FieldInfo(alias="referenceID")
    """
    The unique ID you use internally to refer to this data point, propagated from
    the original function input.
    """

    transformed_content: object = FieldInfo(alias="transformedContent")
    """The transformed content of the input.

    The structure of this object is defined by the function configuration.
    """

    avg_confidence: Optional[float] = FieldInfo(alias="avgConfidence", default=None)
    """Average confidence score across all extracted fields, in the range [0, 1]."""

    call_id: Optional[str] = FieldInfo(alias="callID", default=None)
    """Unique identifier of workflow call that this event is associated with."""

    corrected_content: Optional[TransformEventCorrectedContent] = FieldInfo(alias="correctedContent", default=None)
    """Corrected feedback provided for fine-tuning purposes."""

    created_at: Optional[datetime] = FieldInfo(alias="createdAt", default=None)
    """Timestamp indicating when the event was created."""

    event_type: Optional[Literal["transform"]] = FieldInfo(alias="eventType", default=None)

    field_confidences: Optional[object] = FieldInfo(alias="fieldConfidences", default=None)
    """Per-field confidence scores.

    A JSON object mapping RFC 6901 JSON Pointer paths (e.g. `"/invoiceNumber"`) to
    float values in the range [0, 1] indicating the model's confidence in each
    extracted field value.
    """

    function_call_id: Optional[str] = FieldInfo(alias="functionCallID", default=None)
    """Unique identifier of function call that this event is associated with."""

    function_call_try_number: Optional[int] = FieldInfo(alias="functionCallTryNumber", default=None)
    """The attempt number of the function call that created this event. 1 indexed."""

    function_version_num: Optional[int] = FieldInfo(alias="functionVersionNum", default=None)
    """Version number of function that this event is associated with."""

    inbound_email: Optional[InboundEmailEvent] = FieldInfo(alias="inboundEmail", default=None)
    """The inbound email that triggered this event."""

    inputs: Optional[List[TransformEventInput]] = None
    """Array of transformation inputs with their types and S3 URLs."""

    input_type: Optional[InputType] = FieldInfo(alias="inputType", default=None)
    """The input type of the content you're sending for transformation."""

    invalid_properties: Optional[List[str]] = FieldInfo(alias="invalidProperties", default=None)
    """List of properties that were invalid in the input."""

    is_regression: Optional[bool] = FieldInfo(alias="isRegression", default=None)
    """Indicates whether this transformation was created as part of a regression test."""

    last_publish_error_at: Optional[str] = FieldInfo(alias="lastPublishErrorAt", default=None)
    """
    Last timestamp indicating when the transform was published via webhook and
    received a non-200 response. Set to `null` on a subsequent retry if the webhook
    service receives a 200 response.
    """

    metadata: Optional[TransformEventMetadata] = None

    metrics: Optional[TransformEventMetrics] = None
    """Accuracy, precision, recall, and F1 score when corrected JSON is provided."""

    order_matching: Optional[bool] = FieldInfo(alias="orderMatching", default=None)
    """
    Indicates whether array order matters when comparing corrected JSON with
    extracted JSON.
    """

    pipeline_id: Optional[str] = FieldInfo(alias="pipelineID", default=None)
    """ID of pipeline that transformed the original input data."""

    published_at: Optional[datetime] = FieldInfo(alias="publishedAt", default=None)
    """
    Timestamp indicating when the transform was published via webhook and received a
    successful 200 response. Value is `null` if the transformation hasn't been sent.
    """

    s3_url: Optional[str] = FieldInfo(alias="s3URL", default=None)
    """Presigned S3 URL for the input content uploaded to S3."""

    transformation_id: Optional[str] = FieldInfo(alias="transformationID", default=None)
    """
    Unique ID for each transformation output generated by bem following Segment's
    KSUID conventions.
    """

    workflow_id: Optional[str] = FieldInfo(alias="workflowID", default=None)
    """Unique identifier of workflow that this event is associated with."""

    workflow_name: Optional[str] = FieldInfo(alias="workflowName", default=None)
    """Name of workflow that this event is associated with."""

    workflow_version_num: Optional[int] = FieldInfo(alias="workflowVersionNum", default=None)
    """Version number of workflow that this event is associated with."""


class ExtractEventCorrectedContentOutput(BaseModel):
    output: Optional[List[Optional[AnyType]]] = None


ExtractEventCorrectedContent: TypeAlias = Union[
    ExtractEventCorrectedContentOutput, List[object], str, float, bool, object, None
]


class ExtractEventInput(BaseModel):
    input_content: Optional[str] = FieldInfo(alias="inputContent", default=None)

    input_type: Optional[str] = FieldInfo(alias="inputType", default=None)

    json_input_content: Optional[object] = FieldInfo(alias="jsonInputContent", default=None)

    s3_url: Optional[str] = FieldInfo(alias="s3URL", default=None)


class ExtractEventMetadata(BaseModel):
    duration_function_to_event_seconds: Optional[float] = FieldInfo(
        alias="durationFunctionToEventSeconds", default=None
    )


class ExtractEvent(BaseModel):
    """V3 event variants that do not exist in the shared `Event` union.

    `ExtractEvent` and `ClassifyEvent` are emitted only by V3-era function types
    (`extract` and `classify`). The shared `Event` union in `specs/events/models.tsp`
    predates these types and continues to describe V2 / V1-alpha responses verbatim;
    V3 response payloads add the new variants via the `EventV3` union below while
    keeping every shared variant intact for backward compatibility.
    """

    event_id: str = FieldInfo(alias="eventID")
    """Unique ID generated by bem to identify the event."""

    function_id: str = FieldInfo(alias="functionID")
    """Unique identifier of function that this event is associated with."""

    function_name: str = FieldInfo(alias="functionName")
    """Unique name of function that this event is associated with."""

    item_count: int = FieldInfo(alias="itemCount")
    """The number of items that were transformed.

    Used for batch transformations to indicate how many items were transformed.
    """

    item_offset: int = FieldInfo(alias="itemOffset")
    """The offset of the first item that was transformed.

    Used for batch transformations to indicate which item in the batch this event
    corresponds to.
    """

    reference_id: str = FieldInfo(alias="referenceID")
    """
    The unique ID you use internally to refer to this data point, propagated from
    the original function input.
    """

    transformed_content: object = FieldInfo(alias="transformedContent")
    """The transformed content of the input.

    The structure of this object is defined by the function configuration.
    """

    avg_confidence: Optional[float] = FieldInfo(alias="avgConfidence", default=None)
    """Average confidence score across all extracted fields, in the range [0, 1]."""

    call_id: Optional[str] = FieldInfo(alias="callID", default=None)
    """Unique identifier of workflow call that this event is associated with."""

    corrected_content: Optional[ExtractEventCorrectedContent] = FieldInfo(alias="correctedContent", default=None)
    """Corrected feedback provided for fine-tuning purposes."""

    created_at: Optional[datetime] = FieldInfo(alias="createdAt", default=None)
    """Timestamp indicating when the event was created."""

    event_type: Optional[Literal["extract"]] = FieldInfo(alias="eventType", default=None)

    field_bounding_boxes: Optional[object] = FieldInfo(alias="fieldBoundingBoxes", default=None)
    """Per-field bounding boxes.

    A JSON object mapping RFC 6901 JSON Pointer paths (e.g. `"/invoiceNumber"`,
    `"/items/0/price"`) to the document regions from which each extracted value was
    sourced.
    """

    field_confidences: Optional[object] = FieldInfo(alias="fieldConfidences", default=None)
    """Per-field confidence scores.

    A JSON object mapping RFC 6901 JSON Pointer paths (e.g. `"/invoiceNumber"`) to
    float values in the range [0, 1] indicating the model's confidence in each
    extracted field value.
    """

    function_call_id: Optional[str] = FieldInfo(alias="functionCallID", default=None)
    """Unique identifier of function call that this event is associated with."""

    function_call_try_number: Optional[int] = FieldInfo(alias="functionCallTryNumber", default=None)
    """The attempt number of the function call that created this event. 1 indexed."""

    function_version_num: Optional[int] = FieldInfo(alias="functionVersionNum", default=None)
    """Version number of function that this event is associated with."""

    inbound_email: Optional[InboundEmailEvent] = FieldInfo(alias="inboundEmail", default=None)
    """The inbound email that triggered this event."""

    inputs: Optional[List[ExtractEventInput]] = None
    """Array of transformation inputs with their types and S3 URLs."""

    input_type: Optional[InputType] = FieldInfo(alias="inputType", default=None)
    """The input type of the content you're sending for transformation."""

    invalid_properties: Optional[List[str]] = FieldInfo(alias="invalidProperties", default=None)
    """List of properties that were invalid in the input."""

    metadata: Optional[ExtractEventMetadata] = None

    s3_url: Optional[str] = FieldInfo(alias="s3URL", default=None)
    """Presigned S3 URL for the input content uploaded to S3."""

    transformation_id: Optional[str] = FieldInfo(alias="transformationID", default=None)
    """
    Unique ID for each transformation output generated by bem following Segment's
    KSUID conventions.
    """

    workflow_id: Optional[str] = FieldInfo(alias="workflowID", default=None)
    """Unique identifier of workflow that this event is associated with."""

    workflow_name: Optional[str] = FieldInfo(alias="workflowName", default=None)
    """Name of workflow that this event is associated with."""

    workflow_version_num: Optional[int] = FieldInfo(alias="workflowVersionNum", default=None)
    """Version number of workflow that this event is associated with."""


class ParseEventCorrectedContentOutput(BaseModel):
    output: Optional[List[Optional[AnyType]]] = None


ParseEventCorrectedContent: TypeAlias = Union[
    ParseEventCorrectedContentOutput, List[object], str, float, bool, object, None
]


class ParseEventInput(BaseModel):
    input_content: Optional[str] = FieldInfo(alias="inputContent", default=None)

    input_type: Optional[str] = FieldInfo(alias="inputType", default=None)

    json_input_content: Optional[object] = FieldInfo(alias="jsonInputContent", default=None)

    s3_url: Optional[str] = FieldInfo(alias="s3URL", default=None)


class ParseEventMetadata(BaseModel):
    duration_function_to_event_seconds: Optional[float] = FieldInfo(
        alias="durationFunctionToEventSeconds", default=None
    )


class ParseEvent(BaseModel):
    """Emitted when a `parse` function completes.

    Reuses the `extract` event shape
    on the wire — both wrap a Transformation and downstream consumers care
    about the same `transformedContent` payload — but uses a distinct
    `eventType` discriminator so receivers can dispatch on the function type
    that produced it.
    """

    event_id: str = FieldInfo(alias="eventID")
    """Unique ID generated by bem to identify the event."""

    function_id: str = FieldInfo(alias="functionID")
    """Unique identifier of function that this event is associated with."""

    function_name: str = FieldInfo(alias="functionName")
    """Unique name of function that this event is associated with."""

    item_count: int = FieldInfo(alias="itemCount")
    """The number of items that were parsed.

    Used for batch parsing to indicate how many items were parsed.
    """

    item_offset: int = FieldInfo(alias="itemOffset")
    """The offset of the first item that was parsed.

    Used for batch parsing to indicate which item in the batch this event
    corresponds to.
    """

    reference_id: str = FieldInfo(alias="referenceID")
    """
    The unique ID you use internally to refer to this data point, propagated from
    the original function input.
    """

    transformed_content: object = FieldInfo(alias="transformedContent")
    """The parsed content of the input.

    Top-level keys are `sections`, `entities`, and `relationships`; the precise
    shape is determined by the parse function's configuration.
    """

    avg_confidence: Optional[float] = FieldInfo(alias="avgConfidence", default=None)
    """Average confidence score across all parsed fields, in the range [0, 1]."""

    call_id: Optional[str] = FieldInfo(alias="callID", default=None)
    """Unique identifier of workflow call that this event is associated with."""

    corrected_content: Optional[ParseEventCorrectedContent] = FieldInfo(alias="correctedContent", default=None)
    """Corrected feedback provided for fine-tuning purposes."""

    created_at: Optional[datetime] = FieldInfo(alias="createdAt", default=None)
    """Timestamp indicating when the event was created."""

    event_type: Optional[Literal["parse"]] = FieldInfo(alias="eventType", default=None)

    field_bounding_boxes: Optional[object] = FieldInfo(alias="fieldBoundingBoxes", default=None)
    """Per-field bounding boxes.

    A JSON object mapping RFC 6901 JSON Pointer paths to the document regions from
    which each parsed value was sourced.
    """

    field_confidences: Optional[object] = FieldInfo(alias="fieldConfidences", default=None)
    """Per-field confidence scores.

    A JSON object mapping RFC 6901 JSON Pointer paths to float values in the range
    [0, 1] indicating the model's confidence in each parsed field value.
    """

    function_call_id: Optional[str] = FieldInfo(alias="functionCallID", default=None)
    """Unique identifier of function call that this event is associated with."""

    function_call_try_number: Optional[int] = FieldInfo(alias="functionCallTryNumber", default=None)
    """The attempt number of the function call that created this event. 1 indexed."""

    function_version_num: Optional[int] = FieldInfo(alias="functionVersionNum", default=None)
    """Version number of function that this event is associated with."""

    inbound_email: Optional[InboundEmailEvent] = FieldInfo(alias="inboundEmail", default=None)
    """The inbound email that triggered this event."""

    inputs: Optional[List[ParseEventInput]] = None
    """Array of parse inputs with their types and S3 URLs."""

    input_type: Optional[InputType] = FieldInfo(alias="inputType", default=None)
    """The input type of the content you're sending for transformation."""

    invalid_properties: Optional[List[str]] = FieldInfo(alias="invalidProperties", default=None)
    """List of properties that were invalid in the input."""

    metadata: Optional[ParseEventMetadata] = None

    s3_url: Optional[str] = FieldInfo(alias="s3URL", default=None)
    """Presigned S3 URL for the input content uploaded to S3."""

    transformation_id: Optional[str] = FieldInfo(alias="transformationID", default=None)
    """
    Unique ID for each transformation output generated by bem following Segment's
    KSUID conventions.
    """

    workflow_id: Optional[str] = FieldInfo(alias="workflowID", default=None)
    """Unique identifier of workflow that this event is associated with."""

    workflow_name: Optional[str] = FieldInfo(alias="workflowName", default=None)
    """Name of workflow that this event is associated with."""

    workflow_version_num: Optional[int] = FieldInfo(alias="workflowVersionNum", default=None)
    """Version number of workflow that this event is associated with."""


class AnalyzeEventMetadata(BaseModel):
    duration_function_to_event_seconds: Optional[float] = FieldInfo(
        alias="durationFunctionToEventSeconds", default=None
    )


class AnalyzeEvent(BaseModel):
    """
    Emitted by functions of the legacy `analyze` type (the vision path predecessor of
    `extract`). Carries the extracted JSON along with per-field bounding-box metadata
    identifying the document regions each value was extracted from.
    """

    event_id: str = FieldInfo(alias="eventID")
    """Unique ID generated by bem to identify the event."""

    function_id: str = FieldInfo(alias="functionID")
    """Unique identifier of function that this event is associated with."""

    function_name: str = FieldInfo(alias="functionName")
    """Unique name of function that this event is associated with."""

    invalid_properties: List[str] = FieldInfo(alias="invalidProperties")
    """List of properties that were invalid in the input."""

    reference_id: str = FieldInfo(alias="referenceID")
    """
    The unique ID you use internally to refer to this data point, propagated from
    the original function input.
    """

    transformed_content: object = FieldInfo(alias="transformedContent")
    """The extracted content of the input.

    The structure of this object is defined by the function's `outputSchema`.
    """

    avg_confidence: Optional[float] = FieldInfo(alias="avgConfidence", default=None)
    """Average confidence score across all extracted fields, in the range [0, 1]."""

    call_id: Optional[str] = FieldInfo(alias="callID", default=None)
    """Unique identifier of workflow call that this event is associated with."""

    created_at: Optional[datetime] = FieldInfo(alias="createdAt", default=None)
    """Timestamp indicating when the event was created."""

    event_type: Optional[Literal["analyze"]] = FieldInfo(alias="eventType", default=None)

    field_bounding_boxes: Optional[object] = FieldInfo(alias="fieldBoundingBoxes", default=None)
    """Per-field bounding boxes.

    A JSON object mapping RFC 6901 JSON Pointer paths (e.g. `"/invoiceNumber"`,
    `"/items/0/price"`) to the document regions from which each extracted value was
    sourced.
    """

    field_confidences: Optional[object] = FieldInfo(alias="fieldConfidences", default=None)
    """Per-field confidence scores.

    A JSON object mapping RFC 6901 JSON Pointer paths to float values in the range
    [0, 1] indicating the model's confidence in each extracted field value.
    """

    function_call_id: Optional[str] = FieldInfo(alias="functionCallID", default=None)
    """Unique identifier of function call that this event is associated with."""

    function_call_try_number: Optional[int] = FieldInfo(alias="functionCallTryNumber", default=None)
    """The attempt number of the function call that created this event. 1 indexed."""

    function_version_num: Optional[int] = FieldInfo(alias="functionVersionNum", default=None)
    """Version number of function that this event is associated with."""

    inbound_email: Optional[InboundEmailEvent] = FieldInfo(alias="inboundEmail", default=None)
    """The inbound email that triggered this event."""

    metadata: Optional[AnalyzeEventMetadata] = None

    s3_url: Optional[str] = FieldInfo(alias="s3URL", default=None)
    """Presigned S3 URL of the input file that was analyzed."""

    transformation_id: Optional[str] = FieldInfo(alias="transformationID", default=None)
    """
    Unique ID for each transformation output generated by bem following Segment's
    KSUID conventions.
    """

    workflow_id: Optional[str] = FieldInfo(alias="workflowID", default=None)
    """Unique identifier of workflow that this event is associated with."""

    workflow_name: Optional[str] = FieldInfo(alias="workflowName", default=None)
    """Name of workflow that this event is associated with."""

    workflow_version_num: Optional[int] = FieldInfo(alias="workflowVersionNum", default=None)
    """Version number of workflow that this event is associated with."""


class RouteEventMetadata(BaseModel):
    duration_function_to_event_seconds: Optional[float] = FieldInfo(
        alias="durationFunctionToEventSeconds", default=None
    )


class RouteEvent(BaseModel):
    choice: str
    """The choice made by the router function."""

    event_id: str = FieldInfo(alias="eventID")
    """Unique ID generated by bem to identify the event."""

    function_id: str = FieldInfo(alias="functionID")
    """Unique identifier of function that this event is associated with."""

    function_name: str = FieldInfo(alias="functionName")
    """Unique name of function that this event is associated with."""

    reference_id: str = FieldInfo(alias="referenceID")
    """
    The unique ID you use internally to refer to this data point, propagated from
    the original function input.
    """

    call_id: Optional[str] = FieldInfo(alias="callID", default=None)
    """Unique identifier of workflow call that this event is associated with."""

    created_at: Optional[datetime] = FieldInfo(alias="createdAt", default=None)
    """Timestamp indicating when the event was created."""

    event_type: Optional[Literal["route"]] = FieldInfo(alias="eventType", default=None)

    function_call_id: Optional[str] = FieldInfo(alias="functionCallID", default=None)
    """Unique identifier of function call that this event is associated with."""

    function_call_try_number: Optional[int] = FieldInfo(alias="functionCallTryNumber", default=None)
    """The attempt number of the function call that created this event. 1 indexed."""

    function_version_num: Optional[int] = FieldInfo(alias="functionVersionNum", default=None)
    """Version number of function that this event is associated with."""

    inbound_email: Optional[InboundEmailEvent] = FieldInfo(alias="inboundEmail", default=None)
    """The inbound email that triggered this event."""

    metadata: Optional[RouteEventMetadata] = None

    s3_url: Optional[str] = FieldInfo(alias="s3URL", default=None)
    """The presigned S3 URL of the file that was routed."""

    workflow_id: Optional[str] = FieldInfo(alias="workflowID", default=None)
    """Unique identifier of workflow that this event is associated with."""

    workflow_name: Optional[str] = FieldInfo(alias="workflowName", default=None)
    """Name of workflow that this event is associated with."""

    workflow_version_num: Optional[int] = FieldInfo(alias="workflowVersionNum", default=None)
    """Version number of workflow that this event is associated with."""


class ClassifyEventMetadata(BaseModel):
    duration_function_to_event_seconds: Optional[float] = FieldInfo(
        alias="durationFunctionToEventSeconds", default=None
    )


class ClassifyEvent(BaseModel):
    choice: str
    """The classification chosen by the classify function."""

    event_id: str = FieldInfo(alias="eventID")
    """Unique ID generated by bem to identify the event."""

    function_id: str = FieldInfo(alias="functionID")
    """Unique identifier of function that this event is associated with."""

    function_name: str = FieldInfo(alias="functionName")
    """Unique name of function that this event is associated with."""

    reference_id: str = FieldInfo(alias="referenceID")
    """
    The unique ID you use internally to refer to this data point, propagated from
    the original function input.
    """

    call_id: Optional[str] = FieldInfo(alias="callID", default=None)
    """Unique identifier of workflow call that this event is associated with."""

    created_at: Optional[datetime] = FieldInfo(alias="createdAt", default=None)
    """Timestamp indicating when the event was created."""

    event_type: Optional[Literal["classify"]] = FieldInfo(alias="eventType", default=None)

    function_call_id: Optional[str] = FieldInfo(alias="functionCallID", default=None)
    """Unique identifier of function call that this event is associated with."""

    function_call_try_number: Optional[int] = FieldInfo(alias="functionCallTryNumber", default=None)
    """The attempt number of the function call that created this event. 1 indexed."""

    function_version_num: Optional[int] = FieldInfo(alias="functionVersionNum", default=None)
    """Version number of function that this event is associated with."""

    inbound_email: Optional[InboundEmailEvent] = FieldInfo(alias="inboundEmail", default=None)
    """The inbound email that triggered this event."""

    metadata: Optional[ClassifyEventMetadata] = None

    s3_url: Optional[str] = FieldInfo(alias="s3URL", default=None)
    """The presigned S3 URL of the file that was classified."""

    workflow_id: Optional[str] = FieldInfo(alias="workflowID", default=None)
    """Unique identifier of workflow that this event is associated with."""

    workflow_name: Optional[str] = FieldInfo(alias="workflowName", default=None)
    """Name of workflow that this event is associated with."""

    workflow_version_num: Optional[int] = FieldInfo(alias="workflowVersionNum", default=None)
    """Version number of workflow that this event is associated with."""


class SplitCollectionEventPrintPageOutputItem(BaseModel):
    item_offset: Optional[int] = FieldInfo(alias="itemOffset", default=None)

    item_reference_id: Optional[str] = FieldInfo(alias="itemReferenceID", default=None)

    s3_url: Optional[str] = FieldInfo(alias="s3URL", default=None)


class SplitCollectionEventPrintPageOutput(BaseModel):
    item_count: Optional[int] = FieldInfo(alias="itemCount", default=None)

    items: Optional[List[SplitCollectionEventPrintPageOutputItem]] = None


class SplitCollectionEventSemanticPageOutputItem(BaseModel):
    item_class: Optional[str] = FieldInfo(alias="itemClass", default=None)

    item_class_count: Optional[int] = FieldInfo(alias="itemClassCount", default=None)

    item_class_offset: Optional[int] = FieldInfo(alias="itemClassOffset", default=None)

    item_offset: Optional[int] = FieldInfo(alias="itemOffset", default=None)

    item_reference_id: Optional[str] = FieldInfo(alias="itemReferenceID", default=None)

    page_end: Optional[int] = FieldInfo(alias="pageEnd", default=None)

    page_start: Optional[int] = FieldInfo(alias="pageStart", default=None)

    s3_url: Optional[str] = FieldInfo(alias="s3URL", default=None)


class SplitCollectionEventSemanticPageOutput(BaseModel):
    item_count: Optional[int] = FieldInfo(alias="itemCount", default=None)

    items: Optional[List[SplitCollectionEventSemanticPageOutputItem]] = None

    page_count: Optional[int] = FieldInfo(alias="pageCount", default=None)


class SplitCollectionEventMetadata(BaseModel):
    duration_function_to_event_seconds: Optional[float] = FieldInfo(
        alias="durationFunctionToEventSeconds", default=None
    )


class SplitCollectionEvent(BaseModel):
    event_id: str = FieldInfo(alias="eventID")
    """Unique ID generated by bem to identify the event."""

    function_id: str = FieldInfo(alias="functionID")
    """Unique identifier of function that this event is associated with."""

    function_name: str = FieldInfo(alias="functionName")
    """Unique name of function that this event is associated with."""

    output_type: Literal["print_page", "semantic_page"] = FieldInfo(alias="outputType")

    print_page_output: SplitCollectionEventPrintPageOutput = FieldInfo(alias="printPageOutput")

    reference_id: str = FieldInfo(alias="referenceID")
    """
    The unique ID you use internally to refer to this data point, propagated from
    the original function input.
    """

    semantic_page_output: SplitCollectionEventSemanticPageOutput = FieldInfo(alias="semanticPageOutput")

    call_id: Optional[str] = FieldInfo(alias="callID", default=None)
    """Unique identifier of workflow call that this event is associated with."""

    created_at: Optional[datetime] = FieldInfo(alias="createdAt", default=None)
    """Timestamp indicating when the event was created."""

    event_type: Optional[Literal["split_collection"]] = FieldInfo(alias="eventType", default=None)

    function_call_id: Optional[str] = FieldInfo(alias="functionCallID", default=None)
    """Unique identifier of function call that this event is associated with."""

    function_call_try_number: Optional[int] = FieldInfo(alias="functionCallTryNumber", default=None)
    """The attempt number of the function call that created this event. 1 indexed."""

    function_version_num: Optional[int] = FieldInfo(alias="functionVersionNum", default=None)
    """Version number of function that this event is associated with."""

    inbound_email: Optional[InboundEmailEvent] = FieldInfo(alias="inboundEmail", default=None)
    """The inbound email that triggered this event."""

    metadata: Optional[SplitCollectionEventMetadata] = None

    workflow_id: Optional[str] = FieldInfo(alias="workflowID", default=None)
    """Unique identifier of workflow that this event is associated with."""

    workflow_name: Optional[str] = FieldInfo(alias="workflowName", default=None)
    """Name of workflow that this event is associated with."""

    workflow_version_num: Optional[int] = FieldInfo(alias="workflowVersionNum", default=None)
    """Version number of workflow that this event is associated with."""


class SplitItemEventMetadata(BaseModel):
    duration_function_to_event_seconds: Optional[float] = FieldInfo(
        alias="durationFunctionToEventSeconds", default=None
    )


class SplitItemEventPrintPageOutput(BaseModel):
    collection_reference_id: Optional[str] = FieldInfo(alias="collectionReferenceID", default=None)

    item_count: Optional[int] = FieldInfo(alias="itemCount", default=None)

    item_offset: Optional[int] = FieldInfo(alias="itemOffset", default=None)

    s3_url: Optional[str] = FieldInfo(alias="s3URL", default=None)


class SplitItemEventSemanticPageOutput(BaseModel):
    collection_reference_id: Optional[str] = FieldInfo(alias="collectionReferenceID", default=None)

    item_class: Optional[str] = FieldInfo(alias="itemClass", default=None)

    item_class_count: Optional[int] = FieldInfo(alias="itemClassCount", default=None)

    item_class_offset: Optional[int] = FieldInfo(alias="itemClassOffset", default=None)

    item_count: Optional[int] = FieldInfo(alias="itemCount", default=None)

    item_offset: Optional[int] = FieldInfo(alias="itemOffset", default=None)

    page_count: Optional[int] = FieldInfo(alias="pageCount", default=None)

    page_end: Optional[int] = FieldInfo(alias="pageEnd", default=None)

    page_start: Optional[int] = FieldInfo(alias="pageStart", default=None)

    s3_url: Optional[str] = FieldInfo(alias="s3URL", default=None)


class SplitItemEvent(BaseModel):
    event_id: str = FieldInfo(alias="eventID")
    """Unique ID generated by bem to identify the event."""

    function_id: str = FieldInfo(alias="functionID")
    """Unique identifier of function that this event is associated with."""

    function_name: str = FieldInfo(alias="functionName")
    """Unique name of function that this event is associated with."""

    output_type: Literal["print_page", "semantic_page"] = FieldInfo(alias="outputType")

    reference_id: str = FieldInfo(alias="referenceID")
    """
    The unique ID you use internally to refer to this data point, propagated from
    the original function input.
    """

    call_id: Optional[str] = FieldInfo(alias="callID", default=None)
    """Unique identifier of workflow call that this event is associated with."""

    created_at: Optional[datetime] = FieldInfo(alias="createdAt", default=None)
    """Timestamp indicating when the event was created."""

    event_type: Optional[Literal["split_item"]] = FieldInfo(alias="eventType", default=None)

    function_call_id: Optional[str] = FieldInfo(alias="functionCallID", default=None)
    """Unique identifier of function call that this event is associated with."""

    function_call_try_number: Optional[int] = FieldInfo(alias="functionCallTryNumber", default=None)
    """The attempt number of the function call that created this event. 1 indexed."""

    function_version_num: Optional[int] = FieldInfo(alias="functionVersionNum", default=None)
    """Version number of function that this event is associated with."""

    inbound_email: Optional[InboundEmailEvent] = FieldInfo(alias="inboundEmail", default=None)
    """The inbound email that triggered this event."""

    metadata: Optional[SplitItemEventMetadata] = None

    print_page_output: Optional[SplitItemEventPrintPageOutput] = FieldInfo(alias="printPageOutput", default=None)

    semantic_page_output: Optional[SplitItemEventSemanticPageOutput] = FieldInfo(
        alias="semanticPageOutput", default=None
    )

    workflow_id: Optional[str] = FieldInfo(alias="workflowID", default=None)
    """Unique identifier of workflow that this event is associated with."""

    workflow_name: Optional[str] = FieldInfo(alias="workflowName", default=None)
    """Name of workflow that this event is associated with."""

    workflow_version_num: Optional[int] = FieldInfo(alias="workflowVersionNum", default=None)
    """Version number of workflow that this event is associated with."""


class JoinEventItem(BaseModel):
    item_count: int = FieldInfo(alias="itemCount")
    """The number of items that were transformed."""

    item_offset: int = FieldInfo(alias="itemOffset")
    """The offset of the first item that was transformed.

    Used for batch transformations to indicate which item in the batch this event
    corresponds to.
    """

    item_reference_id: str = FieldInfo(alias="itemReferenceID")
    """The unique ID you use internally to refer to this data point."""

    s3_url: Optional[str] = FieldInfo(alias="s3URL", default=None)
    """The presigned S3 URL of the file that was joined."""


class JoinEventMetadata(BaseModel):
    duration_function_to_event_seconds: Optional[float] = FieldInfo(
        alias="durationFunctionToEventSeconds", default=None
    )


class JoinEvent(BaseModel):
    event_id: str = FieldInfo(alias="eventID")
    """Unique ID generated by bem to identify the event."""

    function_id: str = FieldInfo(alias="functionID")
    """Unique identifier of function that this event is associated with."""

    function_name: str = FieldInfo(alias="functionName")
    """Unique name of function that this event is associated with."""

    invalid_properties: List[str] = FieldInfo(alias="invalidProperties")
    """List of properties that were invalid in the input."""

    items: List[JoinEventItem]
    """The items that were joined."""

    join_type: Literal["standard"] = FieldInfo(alias="joinType")
    """The type of join that was performed."""

    reference_id: str = FieldInfo(alias="referenceID")
    """
    The unique ID you use internally to refer to this data point, propagated from
    the original function input.
    """

    transformed_content: object = FieldInfo(alias="transformedContent")
    """The transformed content of the input.

    The structure of this object is defined by the function configuration.
    """

    avg_confidence: Optional[float] = FieldInfo(alias="avgConfidence", default=None)
    """Average confidence score across all extracted fields, in the range [0, 1]."""

    call_id: Optional[str] = FieldInfo(alias="callID", default=None)
    """Unique identifier of workflow call that this event is associated with."""

    created_at: Optional[datetime] = FieldInfo(alias="createdAt", default=None)
    """Timestamp indicating when the event was created."""

    event_type: Optional[Literal["join"]] = FieldInfo(alias="eventType", default=None)

    field_confidences: Optional[object] = FieldInfo(alias="fieldConfidences", default=None)
    """Per-field confidence scores.

    A JSON object mapping RFC 6901 JSON Pointer paths (e.g. `"/invoiceNumber"`) to
    float values in the range [0, 1] indicating the model's confidence in each
    extracted field value.
    """

    function_call_id: Optional[str] = FieldInfo(alias="functionCallID", default=None)
    """Unique identifier of function call that this event is associated with."""

    function_call_try_number: Optional[int] = FieldInfo(alias="functionCallTryNumber", default=None)
    """The attempt number of the function call that created this event. 1 indexed."""

    function_version_num: Optional[int] = FieldInfo(alias="functionVersionNum", default=None)
    """Version number of function that this event is associated with."""

    inbound_email: Optional[InboundEmailEvent] = FieldInfo(alias="inboundEmail", default=None)
    """The inbound email that triggered this event."""

    metadata: Optional[JoinEventMetadata] = None

    transformation_id: Optional[str] = FieldInfo(alias="transformationID", default=None)
    """
    Unique ID for each transformation output generated by bem following Segment's
    KSUID conventions.
    """

    workflow_id: Optional[str] = FieldInfo(alias="workflowID", default=None)
    """Unique identifier of workflow that this event is associated with."""

    workflow_name: Optional[str] = FieldInfo(alias="workflowName", default=None)
    """Name of workflow that this event is associated with."""

    workflow_version_num: Optional[int] = FieldInfo(alias="workflowVersionNum", default=None)
    """Version number of workflow that this event is associated with."""


class EnrichEventMetadata(BaseModel):
    duration_function_to_event_seconds: Optional[float] = FieldInfo(
        alias="durationFunctionToEventSeconds", default=None
    )


class EnrichEvent(BaseModel):
    enriched_content: object = FieldInfo(alias="enrichedContent")
    """The enriched content produced by the enrich function.

    Contains the input data augmented with results from semantic search against
    collections.
    """

    event_id: str = FieldInfo(alias="eventID")
    """Unique ID generated by bem to identify the event."""

    function_id: str = FieldInfo(alias="functionID")
    """Unique identifier of function that this event is associated with."""

    function_name: str = FieldInfo(alias="functionName")
    """Unique name of function that this event is associated with."""

    reference_id: str = FieldInfo(alias="referenceID")
    """
    The unique ID you use internally to refer to this data point, propagated from
    the original function input.
    """

    call_id: Optional[str] = FieldInfo(alias="callID", default=None)
    """Unique identifier of workflow call that this event is associated with."""

    created_at: Optional[datetime] = FieldInfo(alias="createdAt", default=None)
    """Timestamp indicating when the event was created."""

    event_type: Optional[Literal["enrich"]] = FieldInfo(alias="eventType", default=None)

    function_call_id: Optional[str] = FieldInfo(alias="functionCallID", default=None)
    """Unique identifier of function call that this event is associated with."""

    function_call_try_number: Optional[int] = FieldInfo(alias="functionCallTryNumber", default=None)
    """The attempt number of the function call that created this event. 1 indexed."""

    function_version_num: Optional[int] = FieldInfo(alias="functionVersionNum", default=None)
    """Version number of function that this event is associated with."""

    inbound_email: Optional[InboundEmailEvent] = FieldInfo(alias="inboundEmail", default=None)
    """The inbound email that triggered this event."""

    metadata: Optional[EnrichEventMetadata] = None

    workflow_id: Optional[str] = FieldInfo(alias="workflowID", default=None)
    """Unique identifier of workflow that this event is associated with."""

    workflow_name: Optional[str] = FieldInfo(alias="workflowName", default=None)
    """Name of workflow that this event is associated with."""

    workflow_version_num: Optional[int] = FieldInfo(alias="workflowVersionNum", default=None)
    """Version number of workflow that this event is associated with."""


class PayloadShapingEventMetadata(BaseModel):
    duration_function_to_event_seconds: Optional[float] = FieldInfo(
        alias="durationFunctionToEventSeconds", default=None
    )


class PayloadShapingEvent(BaseModel):
    """
    Emitted by `payload_shaping` functions, which restructure JSON payloads
    using JMESPath expressions configured on the function. The shaped result
    is carried in `transformedContent`.
    """

    event_id: str = FieldInfo(alias="eventID")
    """Unique ID generated by bem to identify the event."""

    function_id: str = FieldInfo(alias="functionID")
    """Unique identifier of function that this event is associated with."""

    function_name: str = FieldInfo(alias="functionName")
    """Unique name of function that this event is associated with."""

    reference_id: str = FieldInfo(alias="referenceID")
    """
    The unique ID you use internally to refer to this data point, propagated from
    the original function input.
    """

    transformed_content: object = FieldInfo(alias="transformedContent")
    """
    The reshaped payload produced by applying the function's JMESPath expressions to
    the input data.
    """

    call_id: Optional[str] = FieldInfo(alias="callID", default=None)
    """Unique identifier of workflow call that this event is associated with."""

    created_at: Optional[datetime] = FieldInfo(alias="createdAt", default=None)
    """Timestamp indicating when the event was created."""

    event_type: Optional[Literal["payload_shaping"]] = FieldInfo(alias="eventType", default=None)

    function_call_id: Optional[str] = FieldInfo(alias="functionCallID", default=None)
    """Unique identifier of function call that this event is associated with."""

    function_call_try_number: Optional[int] = FieldInfo(alias="functionCallTryNumber", default=None)
    """The attempt number of the function call that created this event. 1 indexed."""

    function_version_num: Optional[int] = FieldInfo(alias="functionVersionNum", default=None)
    """Version number of function that this event is associated with."""

    inbound_email: Optional[InboundEmailEvent] = FieldInfo(alias="inboundEmail", default=None)
    """The inbound email that triggered this event."""

    metadata: Optional[PayloadShapingEventMetadata] = None

    workflow_id: Optional[str] = FieldInfo(alias="workflowID", default=None)
    """Unique identifier of workflow that this event is associated with."""

    workflow_name: Optional[str] = FieldInfo(alias="workflowName", default=None)
    """Name of workflow that this event is associated with."""

    workflow_version_num: Optional[int] = FieldInfo(alias="workflowVersionNum", default=None)
    """Version number of workflow that this event is associated with."""


class EvaluationEventMetadata(BaseModel):
    duration_function_to_event_seconds: Optional[float] = FieldInfo(
        alias="durationFunctionToEventSeconds", default=None
    )


class EvaluationEvent(BaseModel):
    """
    Emitted when a function-accuracy evaluation completes for a transformation.
    Evaluations are scheduled by `POST /v3/eval` and run asynchronously; this
    event reports the terminal result.
    """

    evaluation_version: str = FieldInfo(alias="evaluationVersion")
    """Version identifier of the evaluation logic that produced this result."""

    event_id: str = FieldInfo(alias="eventID")
    """Unique ID generated by bem to identify the event."""

    function_id: str = FieldInfo(alias="functionID")
    """Unique identifier of function that this event is associated with."""

    function_name: str = FieldInfo(alias="functionName")
    """Unique name of function that this event is associated with."""

    reference_id: str = FieldInfo(alias="referenceID")
    """
    The unique ID you use internally to refer to this data point, propagated from
    the original function input.
    """

    result: object
    """Evaluator output.

    Shape depends on `evaluationVersion` and includes confidence scores, per-field
    hallucination flags, and relevance metrics.
    """

    status: Literal["success", "failed"]
    """Terminal status of the evaluation run."""

    transform_id: str = FieldInfo(alias="transformId")
    """Unique ID of the transformation that was evaluated."""

    call_id: Optional[str] = FieldInfo(alias="callID", default=None)
    """Unique identifier of workflow call that this event is associated with."""

    created_at: Optional[datetime] = FieldInfo(alias="createdAt", default=None)
    """Timestamp indicating when the event was created."""

    error_message: Optional[str] = FieldInfo(alias="errorMessage", default=None)
    """Failure reason populated when `status` is `failed`."""

    event_type: Optional[Literal["evaluation"]] = FieldInfo(alias="eventType", default=None)

    function_call_id: Optional[str] = FieldInfo(alias="functionCallID", default=None)
    """Unique identifier of function call that this event is associated with."""

    function_call_try_number: Optional[int] = FieldInfo(alias="functionCallTryNumber", default=None)
    """The attempt number of the function call that created this event. 1 indexed."""

    function_version_num: Optional[int] = FieldInfo(alias="functionVersionNum", default=None)
    """Version number of function that this event is associated with."""

    inbound_email: Optional[InboundEmailEvent] = FieldInfo(alias="inboundEmail", default=None)
    """The inbound email that triggered this event."""

    metadata: Optional[EvaluationEventMetadata] = None

    workflow_id: Optional[str] = FieldInfo(alias="workflowID", default=None)
    """Unique identifier of workflow that this event is associated with."""

    workflow_name: Optional[str] = FieldInfo(alias="workflowName", default=None)
    """Name of workflow that this event is associated with."""

    workflow_version_num: Optional[int] = FieldInfo(alias="workflowVersionNum", default=None)
    """Version number of workflow that this event is associated with."""


class CollectionProcessingEventMetadata(BaseModel):
    duration_function_to_event_seconds: Optional[float] = FieldInfo(
        alias="durationFunctionToEventSeconds", default=None
    )


class CollectionProcessingEvent(BaseModel):
    collection_id: str = FieldInfo(alias="collectionID")
    """Unique identifier of the collection."""

    collection_name: str = FieldInfo(alias="collectionName")
    """Name/path of the collection."""

    event_id: str = FieldInfo(alias="eventID")
    """Unique ID generated by bem to identify the event."""

    operation: Literal["add", "update"]
    """The operation performed (add or update)."""

    processed_count: int = FieldInfo(alias="processedCount")
    """Number of items successfully processed."""

    reference_id: str = FieldInfo(alias="referenceID")
    """
    The unique ID you use internally to refer to this data point, propagated from
    the original function input.
    """

    status: Literal["success", "failed"]
    """Processing status (success or failed)."""

    collection_item_ids: Optional[List[str]] = FieldInfo(alias="collectionItemIDs", default=None)
    """Array of collection item KSUIDs that were added or updated."""

    created_at: Optional[datetime] = FieldInfo(alias="createdAt", default=None)
    """Timestamp indicating when the event was created."""

    error_message: Optional[str] = FieldInfo(alias="errorMessage", default=None)
    """Error message if processing failed."""

    event_type: Optional[Literal["collection_processing"]] = FieldInfo(alias="eventType", default=None)

    function_call_try_number: Optional[int] = FieldInfo(alias="functionCallTryNumber", default=None)
    """The attempt number of the function call that created this event. 1 indexed."""

    inbound_email: Optional[InboundEmailEvent] = FieldInfo(alias="inboundEmail", default=None)
    """The inbound email that triggered this event."""

    metadata: Optional[CollectionProcessingEventMetadata] = None


class SendEventGoogleDriveOutput(BaseModel):
    """Metadata returned when a Send function delivers to Google Drive."""

    file_name: str = FieldInfo(alias="fileName")
    """Name of the file created in Google Drive."""

    folder_id: str = FieldInfo(alias="folderID")
    """ID of the Google Drive folder the file was placed in."""


class SendEventMetadata(BaseModel):
    duration_function_to_event_seconds: Optional[float] = FieldInfo(
        alias="durationFunctionToEventSeconds", default=None
    )


class SendEventS3Output(BaseModel):
    """Metadata returned when a Send function delivers to an S3 bucket."""

    bucket_name: str = FieldInfo(alias="bucketName")
    """Name of the S3 bucket the payload was written to."""

    key: str
    """Object key under which the payload was stored."""


class SendEventWebhookOutput(BaseModel):
    """Metadata returned when a Send function delivers to a webhook."""

    http_response_body: str = FieldInfo(alias="httpResponseBody")
    """Raw HTTP response body returned by the webhook endpoint."""

    http_status_code: int = FieldInfo(alias="httpStatusCode")
    """HTTP status code returned by the webhook endpoint."""


class SendEvent(BaseModel):
    delivery_status: Literal["success", "skip"] = FieldInfo(alias="deliveryStatus")
    """Outcome of a Send function's delivery attempt."""

    destination_type: SendDestinationType = FieldInfo(alias="destinationType")
    """Destination type for a Send function."""

    event_id: str = FieldInfo(alias="eventID")
    """Unique ID generated by bem to identify the event."""

    function_id: str = FieldInfo(alias="functionID")
    """Unique identifier of function that this event is associated with."""

    function_name: str = FieldInfo(alias="functionName")
    """Unique name of function that this event is associated with."""

    reference_id: str = FieldInfo(alias="referenceID")
    """
    The unique ID you use internally to refer to this data point, propagated from
    the original function input.
    """

    call_id: Optional[str] = FieldInfo(alias="callID", default=None)
    """Unique identifier of workflow call that this event is associated with."""

    created_at: Optional[datetime] = FieldInfo(alias="createdAt", default=None)
    """Timestamp indicating when the event was created."""

    delivered_content: Optional[object] = FieldInfo(alias="deliveredContent", default=None)
    """
    The full protocol event JSON that was delivered — identical to what subscription
    publish would deliver for the same event. For ad-hoc calls with a JSON file
    input, contains the raw input JSON. For ad-hoc calls with a binary file input,
    contains {"s3URL": "<presigned-url>"}.
    """

    event_type: Optional[Literal["send"]] = FieldInfo(alias="eventType", default=None)

    function_call_id: Optional[str] = FieldInfo(alias="functionCallID", default=None)
    """Unique identifier of function call that this event is associated with."""

    function_call_try_number: Optional[int] = FieldInfo(alias="functionCallTryNumber", default=None)
    """The attempt number of the function call that created this event. 1 indexed."""

    function_version_num: Optional[int] = FieldInfo(alias="functionVersionNum", default=None)
    """Version number of function that this event is associated with."""

    google_drive_output: Optional[SendEventGoogleDriveOutput] = FieldInfo(alias="googleDriveOutput", default=None)
    """Metadata returned when a Send function delivers to Google Drive."""

    inbound_email: Optional[InboundEmailEvent] = FieldInfo(alias="inboundEmail", default=None)
    """The inbound email that triggered this event."""

    metadata: Optional[SendEventMetadata] = None

    s3_output: Optional[SendEventS3Output] = FieldInfo(alias="s3Output", default=None)
    """Metadata returned when a Send function delivers to an S3 bucket."""

    webhook_output: Optional[SendEventWebhookOutput] = FieldInfo(alias="webhookOutput", default=None)
    """Metadata returned when a Send function delivers to a webhook."""

    workflow_id: Optional[str] = FieldInfo(alias="workflowID", default=None)
    """Unique identifier of workflow that this event is associated with."""

    workflow_name: Optional[str] = FieldInfo(alias="workflowName", default=None)
    """Name of workflow that this event is associated with."""

    workflow_version_num: Optional[int] = FieldInfo(alias="workflowVersionNum", default=None)
    """Version number of workflow that this event is associated with."""


class RenderEventMetadata(BaseModel):
    duration_function_to_event_seconds: Optional[float] = FieldInfo(
        alias="durationFunctionToEventSeconds", default=None
    )


class RenderEvent(BaseModel):
    docx_render_seconds: float = FieldInfo(alias="docxRenderSeconds")
    """Wall-clock seconds spent generating the output docx through the template."""

    event_id: str = FieldInfo(alias="eventID")
    """Unique ID generated by bem to identify the event."""

    function_id: str = FieldInfo(alias="functionID")
    """Unique identifier of function that this event is associated with."""

    function_name: str = FieldInfo(alias="functionName")
    """Unique name of function that this event is associated with."""

    output_download_url: str = FieldInfo(alias="outputDownloadURL")
    """
    Short-lived presigned HTTPS URL the recipient can GET to download the rendered
    docx. Bearer-token semantics: possession equals access. Treat as a credential —
    do not log full URLs or store beyond the delivery window. The URL expires after
    the platform-configured TTL; the next API serialization mints a fresh one.
    """

    reference_id: str = FieldInfo(alias="referenceID")
    """
    The unique ID you use internally to refer to this data point, propagated from
    the original function input.
    """

    validation_seconds: float = FieldInfo(alias="validationSeconds")
    """
    Wall-clock seconds spent validating the upstream JSON against the doc-type
    schema.
    """

    call_id: Optional[str] = FieldInfo(alias="callID", default=None)
    """Unique identifier of workflow call that this event is associated with."""

    created_at: Optional[datetime] = FieldInfo(alias="createdAt", default=None)
    """Timestamp indicating when the event was created."""

    event_type: Optional[Literal["render"]] = FieldInfo(alias="eventType", default=None)

    function_call_id: Optional[str] = FieldInfo(alias="functionCallID", default=None)
    """Unique identifier of function call that this event is associated with."""

    function_call_try_number: Optional[int] = FieldInfo(alias="functionCallTryNumber", default=None)
    """The attempt number of the function call that created this event. 1 indexed."""

    function_version_num: Optional[int] = FieldInfo(alias="functionVersionNum", default=None)
    """Version number of function that this event is associated with."""

    inbound_email: Optional[InboundEmailEvent] = FieldInfo(alias="inboundEmail", default=None)
    """The inbound email that triggered this event."""

    metadata: Optional[RenderEventMetadata] = None

    workflow_id: Optional[str] = FieldInfo(alias="workflowID", default=None)
    """Unique identifier of workflow that this event is associated with."""

    workflow_name: Optional[str] = FieldInfo(alias="workflowName", default=None)
    """Name of workflow that this event is associated with."""

    workflow_version_num: Optional[int] = FieldInfo(alias="workflowVersionNum", default=None)
    """Version number of workflow that this event is associated with."""


Event: TypeAlias = Annotated[
    Union[
        TransformEvent,
        ExtractEvent,
        ParseEvent,
        AnalyzeEvent,
        RouteEvent,
        ClassifyEvent,
        SplitCollectionEvent,
        SplitItemEvent,
        ErrorEvent,
        JoinEvent,
        EnrichEvent,
        PayloadShapingEvent,
        EvaluationEvent,
        CollectionProcessingEvent,
        SendEvent,
        RenderEvent,
    ],
    PropertyInfo(discriminator="event_type"),
]
